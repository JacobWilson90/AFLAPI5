...
getLadder <- function(seasonId,roundNumber,leagueId=1,levelId=1,...){
    round_code=NULL
    if(missing(seasonId)) seasonId <- getCurrentSeason(leagueId,levelId,...)
    if(!missing(roundNumber)) round_code <- paste('/',roundNumber)
    cdAPI(paste(paste('leagues',leagueId,'levels',levelId,'seasons',seasonId,'ladder',sep='/'),round_code,sep=''),...) %>%
...
getSquadLists <- function(seasonId,leagueId=1,levelId=1,...){
    ...
    x <- NULL
    for(squad in squads) x <- x %>% bind_rows(getSquadPersons(squad,...))
    return(x %>% arrange(squad.name,name))
}
